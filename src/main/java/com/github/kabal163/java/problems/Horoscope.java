...
/*
Мне пришли гениальная идея! Давайте сделаем астрологический прогноз, и
будем продавать его за деньги! Это же гениально! Ваша программа должна
содержать гороскопы для каждого знака зодиака (хардкодим). Для каждого
знака зодиака нужно иметь по 2 гороскопа. На вход программа принимает
через аргументы командной строки название знака зодиака и подбирает для
него подходящий гороскоп. Программа должна выбрать рандомный (с вероятностью 50%)
гороскоп из двух подходящих и вывести его в консоль.
 */
public class Horoscope {
     ...
     * это множество других массивов - это не что иное, как прогнозы для конкретного знака зодиака -
     * по два для каждого (положительный и негативный).
     * Важно, чтобы индекс подмассива соответствовал {@link AstrologicalSign#code коду} нашего знака зодиака
     */
    ...
    public static void main(String[] args) {
        if (args.length != 1) {
            throw new IllegalArgumentException("В качестве аргумента должен быть передан ровно один знак зодиака");
        }

        // Распознаем (парсим) входящее значение и на его основе создаем объект типа AstrologicalSign
    ...
    /**
     * Пример того, как можно определить с вероятностью 50% нужный нам прогноз
     * для определенного знака зодиака.
     * Каждый знак зодиака имеет код, который мы для него определили. Этот код
     * мы используем в качестве индекса, по которому из массива прогнозов мы
     * получаем соответствующий нашему знаку зодиака.
     *
    ...
    public static void arrayApproach(AstrologicalSign sign) {
        ...
        // В качестве первого индекса используем код знака зодиака - получаем массив прогнозов для этого знака
        // В качестве второго индекса используем псевдослучайное значение, которое вернет нам конкретный прогноз
    ...
    /**
     * Пример с использованием конструкции switch-case.
     * В данном случае, она притянута зауши и не имеет смысла, так как
     * у нашего {@code sign} и так есть код, по которому мы можем достать
     * нужный нам подмассив с прогнозами. Данный пример - это больше
     * демонстрация работы switch-case.
     *
     * @param sign знак зодиака, для которого необходимо показать гороскоп
     */
